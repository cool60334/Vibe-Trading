# backtest-diagnosis Specification

## Purpose

Stage 3 routing logic between backtest results and downstream stages (stage 4 sweep / stage 5 selection). Owns the rules for `proceed` / `back_to_stage_4` / `back_to_stage_2`, the LLM prompt schema fed to `vibe-trading run`, the rule-based fallback heuristic used on LLM failure, and the persisted `diagnosis.json` audit fields. Implemented in `research/pipeline/stage3_diagnose.py` (orchestrator + pure helpers) and `dashboard/server/schemas.py::DiagnosisBlock` (schema contract).

## Requirements

### Requirement: Stage 3-diag walk-forward metrics ingestion

When a strategy's `strategy_runs.json` entry has a non-empty `walk_forward_runs` array, the diagnosis runner SHALL load the metrics from the first walk-forward run's `runs/<wf>/artifacts/metrics.csv` and pass them to both the LLM prompt builder and the rule-based fallback. When `walk_forward_runs` is empty or the metrics file is missing/unreadable, the runner MUST fall back gracefully to train-only behaviour (legacy semantics) with a stdout warning.
