# yaml-hash: 00eb70e1b96c5300813b4bb01139c89a1f01e0f29bfdfb3fb1828217187d7cfe
import sys
...
class SignalEngine:
    ...
    def generate(self, data_map: dict) -> dict:
        ...
        atr_volatility = _factors["atr_volatility"]
        atr_volatility = atr_volatility.reindex(ohlcv.index, method='ffill')
        atr_volatility = atr_volatility.rolling(3, min_periods=1).mean()

        # Entry signals
        entry_long = ((((stablecoin_supply_zscore.rolling(90*24, min_periods=90*24//2).rank(pct=True)*100 >= 80.0)).rolling(3).sum() >= 2) | (((bollinger_band_width.rolling(90*24, min_periods=90*24//2).rank(pct=True)*100 >= 80.0)).rolling(3).sum() >= 2) | (((atr_volatility.rolling(90*24, min_periods=90*24//2).rank(pct=True)*100 <= 20.0)).rolling(3).sum() >= 2))
        entry_short = ((((stablecoin_supply_zscore.rolling(90*24, min_periods=90*24//2).rank(pct=True)*100 <= 20.0)).rolling(3).sum() >= 2) | (((bollinger_band_width.rolling(90*24, min_periods=90*24//2).rank(pct=True)*100 <= 20.0)).rolling(3).sum() >= 2) | (((atr_volatility.rolling(90*24, min_periods=90*24//2).rank(pct=True)*100 >= 80.0)).rolling(3).sum() >= 2))
        ...
        _inv_pct_3 = stablecoin_supply_zscore.rolling(90*24, min_periods=90*24//2).rank(pct=True)*100
        _inv_pct_4 = bollinger_band_width.rolling(90*24, min_periods=90*24//2).rank(pct=True)*100
        _inv_pct_5 = atr_volatility.rolling(90*24, min_periods=90*24//2).rank(pct=True)*100
