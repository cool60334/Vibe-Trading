# yaml-hash: 6d26b83d84bcc644cb9a0142a5b07a36494563bef9ae66fa6439285c8ca3dbf6
import sys
...
class SignalEngine:
    ...
    def generate(self, data_map: dict) -> dict:
        ...
        # Entry signals
        entry_long = ((((stablecoin_supply_z.rolling(120*24, min_periods=120*24//2).rank(pct=True)*100 >= 75.0)).rolling(3).sum() >= 2) & (((funding_z.rolling(120*24, min_periods=120*24//2).rank(pct=True)*100 <= 20.0)).rolling(3).sum() >= 2))
        entry_short = ((((stablecoin_supply_z.rolling(120*24, min_periods=120*24//2).rank(pct=True)*100 <= 20.0)).rolling(3).sum() >= 2) & (((funding_z.rolling(120*24, min_periods=120*24//2).rank(pct=True)*100 >= 75.0)).rolling(3).sum() >= 2))
