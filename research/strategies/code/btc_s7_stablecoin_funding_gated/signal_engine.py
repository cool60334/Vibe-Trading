# yaml-hash: e83c78a8344357e026a6d21c7671d3fcb59bf86b96795c3b6fedeac68861560f
import sys
...
class SignalEngine:
    ...
    def generate(self, data_map: dict) -> dict:
        ...
        # Entry signals
        entry_long = ((((stablecoin_supply_z.rolling(90*24, min_periods=90*24//2).rank(pct=True)*100 >= 80.0)).rolling(3).sum() >= 2) & (((funding_z.rolling(90*24, min_periods=90*24//2).rank(pct=True)*100 <= 20.0)).rolling(3).sum() >= 2))
        entry_short = ((((stablecoin_supply_z.rolling(90*24, min_periods=90*24//2).rank(pct=True)*100 <= 20.0)).rolling(3).sum() >= 2) & (((funding_z.rolling(90*24, min_periods=90*24//2).rank(pct=True)*100 >= 80.0)).rolling(3).sum() >= 2))
