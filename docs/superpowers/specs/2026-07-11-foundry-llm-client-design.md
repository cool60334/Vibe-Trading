...
- **Accepted (improves the design) — true hard cap.** agy: checking only between
  ...
  next job still carries per_job=5 → total 10). Fix: give each job a Budget with
  `max_llm_calls = min(per_job, batch_remaining)` via `dataclasses.replace`, so
  the batch total is a strict hard cap, not a bounded overshoot.
...
- **concurrency race** — `reconcile` is a plain `for` loop ([foundry_runner.py:88]),
  so the between-jobs check is sound. Documented as a sequential assumption.
...
main() run --model <cheap-model> --i-will-spend-real-money   (MAX_RETRIES=1 in env)
   ...
   └─ reconcile_foundry_jobs(..., batch_max_llm_calls=N):
         before each job: remaining = N - sum(summary.get("llm_calls_used", 0))
         per-job Budget = replace(budget, max_llm_calls=min(per_job, remaining))  # hard cap
         remaining <= 0 -> stop the whole batch
```

### N1 — the adapter (`OpenRouterCoder`)

Wraps the agent's `ChatOpenAI` to satisfy `LLMCoder.complete(prompt) -> str`.
`.content` from `.invoke()` may be a plain string or a list of content blocks
(`[{"type":"text","text":"..."}]`); the adapter extracts and joins the `text`
blocks rather than `str()`-ing the list (which would inject Python syntax into
the parser's input). Reasoning is already separated by the agent's client, but a
defensive strip guards against a model that inlines `<think>` in content.

### N2 — batch hard cap (reconcile layer, no change to `run_foundry`)

`Budget` is `@dataclass(frozen=True)`, so each job gets
`dataclasses.replace(budget, max_llm_calls=min(per_job, batch_remaining))`. Since
`run_foundry` builds its `ForgeBudget` from `budget.max_llm_calls`, the narrowed
value flows through with no change to `run_foundry`'s internals. The batch total
is a strict hard cap.
...
| `research/hermes/foundry_runner.py` (modify) | `build_llm("openrouter")` returns the coder; `reconcile` gains `batch_max_llm_calls` + dynamic per-job budget; `main` run gains `--model`, `--i-will-spend-real-money`, usage print |
...
**`LLMUnavailable` needs no forge change.** `llm.complete()` is called at
[forge.py:169], *outside* forge's `try` (which starts at line 180), so an
exception from it propagates straight out of forge — it is never mistaken for a
repairable code error and never retried. It exits `run_foundry` (not caught by
`except BudgetExhausted`), is re-raised by `run_foundry_job` after it writes
`status=failed`, and reaches `reconcile`. The only wiring is `reconcile`'s
discriminator: abort the batch on `LLMUnavailable` **or** a bare `SandboxError`
(not `SandboxRunFailed`); continue on anything else.
...
| test | assertion |
|---|---|
| `test_complete_flattens_text_blocks` | invoke returns `[{"type":"text","text":"c"}]` → `complete()` returns `"c"` |
| `test_complete_passes_plain_string_content` | invoke returns a plain string → returned unchanged |
| `test_complete_wraps_auth_error_as_llm_unavailable` | FakeChat.invoke raises an auth error → `complete` raises `LLMUnavailable`, not the raw provider exception |
...
| test | assertion |
|---|---|
| `test_batch_budget_hard_caps_across_jobs` | fake jobs report `llm_calls_used` → each next job's `Budget.max_llm_calls == min(per_job, remaining)`; batch stops at the ceiling |
| `test_batch_budget_tolerates_missing_calls_key` | a summary without the key → `.get(...,0)`, no KeyError |
| `test_llm_unavailable_aborts_whole_batch` | job1 raises `LLMUnavailable` → job2 **not executed** |
...
- **Consistency:** the four error classes in §3 match the adapter behaviour in §1
  and the tests in §4. `LLMUnavailable` is defined once (§2) and used in §3/§4.
  The hard-cap formula `min(per_job, remaining)` appears identically in §1/§2/§4.
...
- **Ambiguity:** "infra vs repairable" is pinned to concrete classes
  ...
  concrete flag; the cap to `min(per_job, remaining)`.
