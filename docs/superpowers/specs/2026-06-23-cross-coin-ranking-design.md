...
`_strategy_row` 目前只吐 in_sample 的 sharpe/dd，沒吐 oos、trades。本案把 OOS 既有資料吐出來 + 前端排序。

> ⚠️ **校正（plan grounding 發現）**：`selected` **不在** `StrategyManifest`（`schemas.py:465-485` 無此欄），它在 `SelectionEntry`（selection.json，stage5 輸出）。per-strategy manifest 的「可部署」訊號是 `gate`（`overall_pass` / `fatal_fail`）—— 也正是 dashboard promote 按鈕的依據。故排名以 **gate（GO = pass 且非 fatal）** 為「可部署」軸，不用不存在的 `m.selected`；真·stage5 selected 若要，可後續 join selection.json（YAGNI 暫不做）。
...
- 預設**排名**：可部署（gate `overall_pass` 且非 `fatal_fail`）優先 → OOS sharpe 降序（null 沉底）→ `fatal` 沉底。
- 欄頭可點改排序。
- `selected` 徽章。
...
加：`sharpe_oos: number | null`、`dd_oos: number | null`、`trades_oos: number | null`、`pf_oos: number | null`。
...
// 預設「可部署性」排名：GO(gate pass 且非 fatal) 頂 → OOS sharpe 降序(null 沉底) → fatal 沉底。
...
排序規則細節：
- `defaultRank`：① **可部署**（`gate_pass===true && gate_fatal===false`）在前；② `gate_fatal===true` 沉到最底；③ 同層 `sharpe_oos` 降序，`null` 視為 -∞（沉底）。
- `compareBy`：選定欄位排序，`null` 永遠沉底（不論 asc/desc）。

### 3.4 前端 `Compare.tsx`

- `StrategyRow` 多欄渲染。表格欄序：
  `策略 │ 幣種 │ 級別 │ OOS Sharpe │ OOS DD │ OOS Trades │ IS Sharpe(灰,次) │ Stage │ GO/NO-GO │ 紅旗`
- state `sortKey`（預設 `"rank"`）、`sortDir`。`rows` 經 `useMemo` 排序後渲染。
- 欄頭（OOS Sharpe / OOS DD / OOS Trades / IS Sharpe / Stage）可點：切該欄排序、再點切方向；顯示 ▲▼ 指示。
- 可部署訊號沿用既有 **GO/NO-GO 徽章**（`gate_pass`/`gate_fatal`），**不另加 Selected 欄**（避免冗餘；manifest 本無 selected）。
...
- **後端 pytest**（`dashboard/server/test_main.py` 或 `test_*`）：
  - `_strategy_row`：manifest 帶 `backtest.oos` → row 有 `sharpe_oos/dd_oos/trades_oos/pf_oos` 正確值。
  - manifest 無 `oos`（`backtest.oos is None`）→ 上述 OOS 欄皆 `None`，不報錯。
  - 無 `backtest` → 全 None 安全。
- **前端 ranking util**：`dashboard/web` **無 vitest**（package.json 僅 dev/build/preview）→ 不為此加測試框架（YAGNI）；`defaultRank`/`compareBy` 靠 `tsc -b` 型別檢查 + preview 行為驗證。
...
2. 預設排序：可部署（GO）在頂、其餘 OOS sharpe 降序、fatal 沉底。
3. 欄頭可點改排序。
...
- 排名依據：可部署（gate GO）優先 → OOS sharpe 降序 → fatal 沉底（非 stage5 composite score、非 `manifest.selected`〔不存在〕；透明直觀，欄頭仍可改排）。
